%%% 13 Jan 2015
%%% Read the exon expression data of DMD

%% Define directories
if ispc
    atlasDir = 'C:/Users/amahfouz/SURFdrive/Projects/DMD/Data/ALLEN BRAIN ATLASES/exons_matrix_csv/'; 
...
transcript.gene_symbol = txt(2:end,4);
transcript.ensembl_gene_id = txt(2:end,3);
transcript.entrez_id = num(:,5);
transcript.start = num(:,6);
transcript.end = num(:,7);
clear num; clear txt;
save([dataDir 'transcript.mat'],'transcript');
...
clear num; clear txt;
save([dataDir 'exon_sample.mat'],'sample');
...
clear data;
save([dataDir 'dmd_exon_data.mat'],'dmd_data','gene_idx');
...
% calculate the correlation
[corrMat pVal] = corr([trans1_exp;trans2_exp;trans3_exp]', data');
save([resDir 'corrMat_exonArray_individual_exons_log2.mat'],'corrMat','pVal','transcript');
...
for i = 1 : size(corrMat,1)
    ...
    % save to excel
    xlswrite([resultsDir geneName '_exonArray_coexpressed_genes_individual_exons_log2.xlsx'], sortedG(2:end), i, 'A1');
    xlswrite([resultsDir geneName '_exonArray_coexpressed_genes_individual_exons_log2.xlsx'], sotredEntrez(2:end), i, 'B1');
    xlswrite([resultsDir geneName '_exonArray_coexpressed_genes_individual_exons_log2.xlsx'], sortedCorr(2:end)', i, 'C1');
...
load([resDir 'corrMat_exonArray_individual_exons_log2.mat']);
removedRows = logical(prod(double(isnan(corrMat))));
corrMat(:,removedRows) = [];
transcript.gene_symbol(removedRows) = [];
transcript.entrez_id(removedRows) = [];
...
save([resDir 'corrMat_exonArray_individual_exons_log2_topN.mat'],'corrMat_topN','topN_genes');

%% load the topN corelations and save to excel
load([dataDir 'corrMat_exonArray_individual_exons_log2_topN.mat'])
for k = 1 : size(corrMat_topN,3)
    ...
    writetable(T, [dataDir 'DMD_exons_individual_topN_log2' num2str(k) '.xls']);
end
